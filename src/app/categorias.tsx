...
export default function CategoriasScreen() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      Alert.alert('No se pudo crear', error?.message ?? 'No fue posible crear la categoría.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = (id: string, categoryName: string) => {
    Alert.alert(
      'Eliminar categoría',
      `¿Eliminar “${categoryName}”? Los gastos anteriores conservarán esa categoría en su historial.`,
      [
      ...
      >
        ...
        >
          ...
          <Text style={styles.subtitle}>
            Crea las categorías que necesites y elimina las que no uses.
          </Text>
          ...
          <Text style={styles.note}>
            Debes conservar al menos una categoría. Eliminar una categoría no borra tus gastos anteriores.
          </Text>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  addButton: { backgroundColor: '#2563EB', borderRadius: 14, padding: 15, alignItems: 'center', marginTop: 20 },
  addButtonText: { color: '#FFFFFF', fontWeight: '800', fontSize: 15 },
  disabled: { opacity: 0.6 },
  headerRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 28, marginBottom: 12 },
  sectionTitle: { color: '#FFFFFF', fontSize: 18, fontWeight: '800' },
  count: { color: '#94A3B8', backgroundColor: '#1E293B', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, overflow: 'hidden' },
  list: { gap: 10 },
  categoryRow: { backgroundColor: '#111827', borderRadius: 16, padding: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  categoryLeft: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  categoryIconBox: { width: 42, height: 42, borderRadius: 12, backgroundColor: '#1E293B', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  categoryIcon: { fontSize: 21 },
  categoryName: { color: '#FFFFFF', fontSize: 15, fontWeight: '700', flex: 1 },
  deleteButton: { paddingVertical: 8, paddingHorizontal: 10 },
  deleteText: { color: '#F87171', fontSize: 13, fontWeight: '700' },
  note: { color: '#64748B', fontSize: 12, lineHeight: 18, marginTop: 18, textAlign: 'center' },
});
